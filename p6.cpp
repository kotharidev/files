...
fstream file; int pos; string buffer,name; 
count=-1;  
file.open("11.txt",ios::in); 
while(!file.eof()) 
{ 
pos=file.tellg();  
buffer.erase();  
...
if(buffer.empty()) 
break; 
name=extract_Name(buffer); Name_list[++count]=name;  
...
{ int i,j,temp_Address; string temp_Name; 
...
{ 
string  USN,Name,Branch,semester,buffer; int pos; 
fstream file ; 
cout<< "\n USN:"; cin>>USN;
cout<< "\n Name:"; cin>>Name; 
cout<< "\nBranch:" ; cin>>Branch; 
...
file.open("11.txt",ios::out|ios::app); 
pos=file.tellp(); file<<buffer; file.close();  
Name_list[++count]=Name; Address_list[count]=pos; 
sort_index(); 
} 
...
{  
char del_ch='*'; int i,address; 
if(pos>=0){ 
fstream file;  
file.open("11.txt");  
address=Address_list[pos]; file.seekp(address,ios::beg); 
file.put(del_ch);  
cout<<endl<<"\n\nRecord deleted:"; 
file.close();  
} 
...
switch(ch){  
case 1:cout<<"Data \n";  
i1.insert(); break; case 2:cout<<"Enter the name\n"; cin>>key; 
i1.search(key); break; case 3:cout<<"Enter the Name"; 
cin>>key; 
 i1.remove(key); 
break; case 4: return 0; default:cout<<"Wrong  Choice!!!!!!!\n\n"; 
